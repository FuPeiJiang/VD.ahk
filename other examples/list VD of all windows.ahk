...
Loop %windows%
{
    ...
    if !(DllCall("IsWindowVisible", "UInt", id))
        ; p(DllCall("IsWindowVisible", "UInt", id))
    continue

    WinGet, Style, Style, %ahk_idId% 
    ; p(Style)
...
; for i in arrayOfWindowsInfo {

    ; if !varIn(v["EXE"],filterEXE)
    ; ArrayStreamArray.push(v["str"])
; }
;sort it by desktopNum

; d(array_tostring(arrayOfWindowsInfo))
